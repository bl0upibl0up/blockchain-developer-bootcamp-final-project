...
import festivalFactory from '../proxies/FestivalMarketPlace';
...
class SecondaryPurchase extends Component {
  constructor() {
    super();
  ...
  updateTickets = async () => {
    try {
      ...
      const nftInstance = await festivalTicket(this.state.fest);
  ...
  onPurchaseTicket = async (ticketId, sellingPrice, initiator) => {
    try {
      ...
      const marketplaceInstance = await transactionsLogic(marketplace);
      await festivalMarketToken.methods.approve(marketplace, sellingPrice).send({ from: initiator, gas: 6700000 });
...
export default SecondaryPurchase;