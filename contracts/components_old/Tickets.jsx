...
class Tickets extends Component {
  ...
  updateFestivals = async () => {
    try {
      ...
      const festDetails = await festivalMarketPlace.methods.getFestDetails(activeFests[0]).call({ from: initiator });
      const renderData = await Promise.all(activeFests.map(async (fest, i) => {
        const festDetails = await festivalMarketPlace.methods.getFestDetails(activeFests[i]).call({ from: initiator });
        return (
